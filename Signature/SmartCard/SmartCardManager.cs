...
namespace SungurTekSignature.Signature.SmartCard
{
   public class SmartCardManager
    {
        ...
        public static SmartCardManager mSCManager;

        public int mSlotCount = 0;

        public readonly String mSerialNumber;

        public ECertificate mSignatureCert;

        public ECertificate mEncryptionCert;

        public IBaseSmartCard bsc;

        public BaseSigner mSigner;
        ...
        private SmartCardManager()
        {
            try
            {
                LicenseUtil.setLicenseXml(new FileStream(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "lisans/lisans.xml"), FileMode.Open, FileAccess.Read));
                ...
                try
                {
                    ...
                    List<byte[]> cert = bsc.getSignatureCertificates();
                    
                }
                catch (Exception e)
                {
                    MessageBox.Show("Kart takılı kart okuyucu bulunamadı");
        ...
        public BaseSigner getSigner(String aCardPIN, ECertificate aCert)
        {
            ...
            {
                bsc.login(aCardPIN);
                mSigner = bsc.getSigner(aCert, Algorithms.SIGNATURE_RSA_SHA512);
            }
            return mSigner;
        }
